            ...
            try
            {
                if (Page.IsValid)
                {
                    ...
                    if (nTournamentId > 0)
                    {
                        ...
                        lblMessage.Text = string.Format("Statistics calculation completed for tournament: <b>[{0}]</b> and for a total of <b>{1}</b> teams!", ddlTournament.SelectedItem.Text, teamCount);
                    }
                }
            }
            catch (Exception ex)
            {
                lblMessage.Text = string.Format("Statistics calculation failed for tournament: <b>[{0}]</b>", ddlTournament.SelectedItem.Text);
