...
namespace Cricket.DAL.Matches
{
    ...
    public class GetSchedule : Command
    {
        public GetSchedule(Connection conn)
            : base(conn)
        {
            m_cmd.CommandText = "select m.*, team1 = t1.name, team2 = t2.name, umpiring_team = t3.name, umpiring_team2 = t5.name, winning_team = t4.name, ground = g.name, umpire1, umpire2, supervisor1, supervisor2 " +
                "from match m  " +
	...
	public class GetMatchData : Command
	{
		...
		{
            m_cmd.CommandText = "select m.*, team1 = t1.name, team2 = t2.name, umpiring_team = t3.name, umpiring_team2 = t5.name, winning_team = t4.name, ground = g.name, m.tournament_id, umpire1, umpire2, supervisor1, supervisor2 " +
				"from match m  " +
				"left join team t1 on m.team1_id = t1.team_id " +
				"left join team t2 on m.team2_id = t2.team_id " +
				"left join team t3 on m.umpiring_team_id = t3.team_id " +
	...
	public class CreateMatchData : Command
	{
		...
		{
            m_cmd.CommandText = "insert into match (match_id, tournament_id, team1_id, team2_id, umpiring_team_id, winning_team_id, type_cd, match_dt, ground_id, result, mom_player_id, start_time, end_time, umpiring_text, umpiring_team2_id, umpire1, umpire2, supervisor1, supervisor2) " +
                                "values (@match_id, @tournament_id, @team1_id, @team2_id, @umpiring_team_id, @winning_team_id, @type_cd, @match_dt, @ground_id, @result, @mom_player_id, @start_time, @end_time, @umpiring_text, @umpiring_team2_id, @umpire1, @umpire2, @supervisor1, @supervisor2) ";
			addParmInt("match_id");
			addParmInt("tournament_id");
			addParmInt("team1_id");
			addParmInt("team2_id");
            ...
            addParmText("umpire1");
            addParmText("umpire2");
            addParmText("supervisor1");
            addParmText("supervisor2");
        }
	}
	...
	public class SetMatchData : Command
	{
		public SetMatchData(Connection conn) : base(conn)
		{
			m_cmd.CommandText = "update match set tournament_id = @tournament_id, team1_id = @team1_id, team2_id = @team2_id, umpiring_team_id = @umpiring_team_id, winning_team_id = @winning_team_id, " +
                ...
                "umpire1 = @umpire1, umpire2 = @umpire2, supervisor1 = @supervisor1, supervisor2 = @supervisor2 " +
				"where match_id = @match_id";
			addParmInt("match_id");
			addParmInt("tournament_id");
			addParmInt("team1_id");
