...
{
	/// <summary>
	/// Summary description for teamPlayerStats.
	/// </summary>
	public partial class teamPlayerStats : PageBaseTournament
	{
	
		protected int		m_nSerialNo = 1;
		protected int		m_nSerialNo2 = 1;
		protected int		m_nTeamId = 0;
		...
		{
			lblTeamName.Text = m_bl.getTeamName(m_nTeamId);

            SqlDataReader dr = m_bl.getAllPlayersBattingStats(tournamentId, m_nTeamId);
			dgrid_batting.DataSource = dr;
			dgrid_batting.DataBind();
			dr.Close();

			dr = m_bl.getAllPlayersBowlingStats(tournamentId, m_nTeamId);
		...
		private void dgrid_bowling_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				e.Item.Cells[0].Text = m_nSerialNo2.ToString();
				m_nSerialNo2++;
			}
		}
		...
		private void dgrid_batting_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{
				Session["back_url"] = "teamPlayerStats.aspx";
		...
		protected void btnTeamStats_Click(object sender, System.EventArgs e)
		{
