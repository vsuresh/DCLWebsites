...
namespace Cricket.Admin
{
	...
	public partial class schedule_edit : PageBaseAdmin
	{
		...
		protected void loadData()
		{
            ...
            int enableWeekendsOnly = toInt(System.Configuration.ConfigurationManager.AppSettings["DisplayWeekendsOnlyforScheduleEntry"]);

            DateTime dt = startDate;
            int totalsDayToDisplay = endDate.Subtract(startDate).Days;
            for (int i = 0; i < totalsDayToDisplay; i++)
			{
				string strDate = dt.ToShortDateString();
				string strDay = "";

                if (dt.DayOfWeek == DayOfWeek.Saturday || dt.DayOfWeek == DayOfWeek.Sunday || enableWeekendsOnly == 0)
				{
                    ...
                    //    strDay = "Sunday";
					ddlMatchDt.Items.Add(new ListItem(strDate + " " + strDay, strDate)); 
				}
				dt = dt.AddDays(1);
			}
		...
		protected void btnSubmit_Click(object sender, System.EventArgs e)
		{
			if (Page.IsValid)
			{
                ...
                if (m_nMatchId > 0)
                {
                    ...
                    m_bl.setMatch(m_nMatchId, toInt(ddlTournament.SelectedValue), toInt(ddlTeam1.SelectedValue), toInt(ddlTeam2.SelectedValue), toInt(ddlUmpireTeam.SelectedValue), toInt(ddlWinTeam.SelectedValue), toInt(ddlTypecd.SelectedValue), DateTime.Parse(ddlMatchDt.SelectedValue), toInt(ddlGround.SelectedValue), txtResult.Text, toInt(ddlMOM.SelectedValue), m_nTournamentMatchOvers, txtStartTime.Text, txtEndTime.Text, umpiringText, toInt(ddlUmpireTeam2.SelectedValue));
                }
                else
                    m_bl.addMatch(toInt(ddlTournament.SelectedValue), toInt(ddlTeam1.SelectedValue), toInt(ddlTeam2.SelectedValue), toInt(ddlUmpireTeam.SelectedValue), toInt(ddlWinTeam.SelectedValue), toInt(ddlTypecd.SelectedValue), DateTime.Parse(ddlMatchDt.SelectedValue), toInt(ddlGround.SelectedValue), txtResult.Text, toInt(ddlMOM.SelectedValue), txtStartTime.Text, txtEndTime.Text, umpiringText, toInt(ddlUmpireTeam2.SelectedValue));
