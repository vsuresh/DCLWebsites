...
namespace Cricket.Team
{
	...
	/// </summary>
	public partial class match_score : PageBaseTeam
	{
		...
		protected void loadData()
		{
			...
			int nInningNo = m_bl.getMatchInningNo(matchId, nTeam1Id);
			if (nInningNo < 2)
			{
			...
			//Inning1 details
			dr = m_bl.getMatchBattingData(matchId, nInning1TeamId);
			dgrid_batting.DataSource = dr;
			dgrid_batting.DataBind();
			dr.Close();

			dr = m_bl.getMatchStats(matchId, nInning1TeamId);
			dgrid_summary.DataSource = dr;
			dgrid_summary.DataBind();
			dr.Close();

			dr = m_bl.getMatchBowlingData(matchId, nInning1TeamId);
			dgrid_bowling2.DataSource = dr;
			dgrid_bowling2.DataBind();
			dr.Close();

			//Inning2 details
			dr = m_bl.getMatchBattingData(matchId, nInning2TeamId);
			dgrid_batting2.DataSource = dr;
			dgrid_batting2.DataBind();
			dr.Close();

			dr = m_bl.getMatchStats(matchId, nInning2TeamId);
			dgrid_summary2.DataSource = dr;
			dgrid_summary2.DataBind();
			dr.Close();

			dr = m_bl.getMatchBowlingData(matchId, nInning2TeamId);
		...
		{
			Server.Transfer(Request.UrlReferrer.PathAndQuery);
		}
