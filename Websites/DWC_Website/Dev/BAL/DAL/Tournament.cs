...
namespace Cricket.DAL.Tournament
{
	...
	public class GetTournamentData : Command
	{
		...
		{
			m_cmd.CommandText = "select tournament_id, name, description, created_dt, start_dt, end_dt, overs, fees, regstart_dt, regend_dt, registration_info, format from tournament where tournament_id = @tournament_id ";
			addParmInt("tournament_id");
		}
	}
	...
	public class CreateTournament : Command
	{
		...
		{
            m_cmd.CommandText = "insert into tournament (tournament_id, name, description, created_dt, start_dt, end_dt, overs, fees, regstart_dt, regend_dt, registration_info, format) " +
                "values (@tournament_id, @name, @description, GetDate(), @start_dt, @end_dt, @overs, @fees, @regstart_dt, @regend_dt, @registration_info, @format) ";
			addParmInt("tournament_id");
			addParmText("name");
	...
	public class SetTournamentData : Command
	{
		public SetTournamentData(Connection conn) : base(conn)
		{
			m_cmd.CommandText = "update tournament set " +
                ...
                "fees = @fees, regstart_dt = @regstart_dt, regend_dt = @regend_dt, registration_info = @registration_info, format = @format " +
